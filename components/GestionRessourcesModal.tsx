...
export default function GestionRessourcesModal({
  ...
  // All base people including removed (for the manage list)
  const allBase = PEOPLE;

  const filteredList = useMemo(() => {
    const q = search.toLowerCase();
    // Show active + custom; show removed with a "supprimé" badge
    const all = [
      ...PEOPLE,
  ...
  const handleGeocode = useCallback(async () => {
    ...
    try {
      const query = encodeURIComponent(
        `${form.ville} ${form.codePostal} France`.trim()
      );
      ...
      const detectedVille =
        address?.city || address?.town || address?.village || form.ville;
      setForm((f) => ({
        ...f,
        ville: detectedVille,
        lat: parseFloat(lat),
        lng: parseFloat(lon),
      } as typeof f & { lat: number; lng: number }));
      setGeocodeError("");
    } catch {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // Geocode if coords not yet resolved
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {(["list", "add"] as const).map((t) => (
            <button
              key={t}
              onClick={() => { setTab(t); setGeocodeError(""); }}
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        border: `1px solid ${removed ? "rgba(239,68,68,0.25)" : "var(--color-border)"}`,
                    ...
                    >
                      ...
                      {removed ? (
                        <button
                          ...
                          className="flex-shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-colors hover:opacity-80"
                        ...
                        <button
                          ...
                          className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center transition-colors hover:bg-red-500/20"
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4">
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    value={form.ville}
                    onChange={(e) => setForm((f) => ({ ...f, ville: e.target.value, lat: undefined, lng: undefined } as typeof f))}
                    className="px-3 py-2.5 rounded-xl text-sm outline-none"
                    style={{ background: "var(--color-surface-elevated)", border: "1px solid var(--color-border)", color: "var(--color-text-primary)" }}
                  />
                </div>
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) => setForm((f) => ({ ...f, codePostal: e.target.value, lat: undefined, lng: undefined } as typeof f))}
                    className="px-3 py-2.5 rounded-xl text-sm outline-none"
                    style={{ background: "var(--color-surface-elevated)", border: "1px solid var(--color-border)", color: "var(--color-text-primary)" }}
                  />
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {ALL_ROLES.map((r) => (
                    <button
                      ...
                      style={{
                        background: form.role === r ? `${ROLE_COLORS[r]}22` : "var(--color-surface-elevated)",
                        border: `1px solid ${form.role === r ? ROLE_COLORS[r] : "var(--color-border)"}`,
                        color: form.role === r ? ROLE_COLORS[r] : "var(--color-text-secondary)",
                      }}
                    >
                      {ROLE_LABELS[r]}
                    </button>
              ...
              <button
                ...
                disabled={geocoding}
                className="flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all active:scale-[0.98]"
                style={{
      ...
      {confirmDelete && (
        ...
        >
          <div
            className="flex flex-col gap-4 rounded-2xl p-6 shadow-2xl w-full"
            style={{
              background: "var(--color-surface)",
              border: "1px solid var(--color-border)",
              maxWidth: "340px",
