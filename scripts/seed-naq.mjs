// One-shot seed script — inserts all 63 NAQ resources into the resources table.
// Run: node --env-file-if-exists=/vercel/share/.env.project scripts/seed-naq.mjs
// Safe to re-run: ON CONFLICT DO NOTHING on deterministic IDs (base-<NOM>).
...
const PEOPLE = [
  ...
  { nom: "GHAIOUT",         prenom: "Abdelkader",     codePostal: "19274", ville: "Ussac",                      role: "TMRe", lat: 45.2950,  lng: 1.5100  },
...
try {
  ...
  for (const p of PEOPLE) {
    const id = `base-NAQ-${p.nom.replace(/\s+/g, "_").toUpperCase()}`;
  ...
  console.log(`[seed-naq] Done — ${inserted} new rows inserted (${PEOPLE.length - inserted} already existed)`);
} finally {
  await client.end();
}
